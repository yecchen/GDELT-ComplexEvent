...
def main(args, generator, doc_list, level1_event_df, exists_results, start_rowid, end_rowid):
    ...
    for idx, row in tqdm(level1_event_df.iterrows(), total=len(level1_event_df)):
        ...
        try:
            ...
            msg = generator.get_prompt_2_relation(article, row)
            ...
            with open(args.output_path + args.year + '/results_level2/' + str(idx), 'w') as fresult:
                fresult.write(rsp)
            ...
            with open(args.output_path + args.year + '/errors_level2/' + str(idx), 'a') as ferror:
                ferror.write('\n---\n')
                ferror.write(str(e))

    # json.dump(all_msgs, open('./outputs/test_msg_1-5_level2.json', 'w'), indent=4)
    # json.dump(all_rsps, open('./outputs/test_rsp_1-5_level2.json', 'w'), indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_rowid', type=int, default=0,
                        help='specify the start rowid in level1 df')
    parser.add_argument('--end_rowid', type=int, default=0,
                        help='specify the end rowid in level1 df')
    ...
    if not os.path.isdir(args.output_path + year + '/results_level2'):
        print('Making new dir: ' + args.output_path + year + '/results_level2')
        print('Making new dir: ' + args.output_path + year + '/errors_level2')
        os.makedirs(args.output_path + year + '/results_level2')
        os.makedirs(args.output_path + year + '/errors_level2')
    else:
        print('Dir exists: '+ args.output_path + year + '/results_level2')
    ...
    # check results
    exists_results = set(os.listdir(args.output_path + year + '/results_level2'))
    ...
    main(args, generator, doc_list, level1_event_df, exists_results, start_rowid, end_rowid)
