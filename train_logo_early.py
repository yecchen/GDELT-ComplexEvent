...
def get_cmd():
    ...
    parser.add_argument("-i", "--info", type=str, default="early", help="additional info for certain run")
...
def test(conf, model, model_name,
    ...
    for batch_idx, query_time in enumerate(tqdm(query_times)):
        ...
        # global forward
        global_ent_embs = model.get_global_ent_embs(global_g_list)
        ...
        for ceid in time2query_ceids[query_time]:
            ...
            # local forward
            local_ent_embs = model.get_local_ent_embs(local_g_list)
            # early fusion in entity space before pass into decoder
            fused_ent_embs = global_ent_embs + local_ent_embs
            local_s_embs = fused_ent_embs[local_triplets[:, 0]]
            all_s_embs.append(local_s_embs)
            all_fused_ent_embs.append(fused_ent_embs)
        all_s_embs = torch.cat(all_s_embs)
        all_query_embs = model.get_query_embs(all_s_embs, global_triplets)
        query_idx = 0
        for idx, ceid in enumerate(time2query_ceids[query_time]):
            local_triplets = torch.LongTensor(test_list_with_ceid[query_time][ceid]).to(device)
            curr_query_embs = all_query_embs[query_idx: query_idx + len(local_triplets)]
            curr_ent_embs = all_fused_ent_embs[idx]
            final_score = model.predict_query(curr_query_embs, curr_ent_embs)
            query_idx += len(local_triplets)

            mrr_filter, rank_filter = utils.get_total_rank(local_triplets, final_score, all_ans_dict[query_time], eval_bz=1000)

            popularity_tag = list(map(lambda x: utils.popularity_map(x, head_ents), local_triplets))
...
def main():
    ...
    if conf['model'] == 'LoGo_share':
        ...
        print('Model: LoGo_share_early')
    elif conf['model'] == 'LoGo_sep':
        model = LoGo_sep(conf)
        print('Model: LoGo_sep_early')
    else:
    ...
    for epoch in range(conf["n_epochs"]):
        ...
        for batch_idx, query_time in enumerate(tqdm(query_times)):
            ...
            for idx, ceid in enumerate(time2query_ceids[query_time]):
                ...
                curr_query_embs = all_query_embs[query_idx: query_idx+len(local_triplets)]
                curr_ent_embs = all_fused_ent_embs[idx]
                loss += model.forward_query(curr_query_embs, curr_ent_embs, local_triplets) * len(local_triplets)
                query_idx += len(local_triplets)
